...
import { baseUrl, searchThreeStarHotels, threeStarHotels } from '../../../apis';
...
const ThreeStarHotel = () => {
    ...
    const getAllHotels = async () => {
        try {
            setLoading(true);
            const data = await fetch(threeStarHotels, {
                method: "GET",
                headers: {
    ...
    const searchFun = async (text) => {
        ...
        if (text.trim() !== "") {
            setIsSearching(true);
            const data = await fetch(searchThreeStarHotels + `/${text}`);
