...
import { baseUrl, employeeList } from '../../../apis';
...
const AgentList = () => {
    ...
    const fetchData = async () => {
        ...
        let data = await fetch(employeeList + `/${userId}`, {
            method: "GET",
            headers: {
    ...
    const renderItem = ({ item }) => {
        ...
        return (
            <TouchableOpacity onPress={() => naviagtion.navigate("AgentDetails", { item })}
                style={styles.buttonStyle}>
                <Image style={styles.imageStyle} source={{ uri: image }} />
                <View>
                    <Text style={styles.nameText}>{item.name}</Text>
                    <Text style={styles.numberText}>Phone Number: {item.number}</Text>
                    <Text style={styles.numberText}>Address: {item.address.length > 30
            ...
            <ImageBackground style={styles.backImage} source={IMAGES.CompanyBackground}>
...

                {
                    ...
                    /> : <View style={styles.noDataView}>
                        <Text style={styles.companyText}>No Registered Employee</Text>
                    </View>
                }
                {
